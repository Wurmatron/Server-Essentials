			...
			if (mode != -1) {
				if (args.length > 1) {
					...
					if (user != null) {
						...
						ChatHelper.sendMessageTo(user,
								Local.MODE_CHANGED.replaceAll("#", GameType.getByID(mode).getName()));
						ChatHelper.sendMessageTo(sender,
								Local.MODE_CHANGED_OTHER.replaceAll("#", user.getDisplayName().getUnformattedText())
										.replaceAll("\\$", GameType.getByID(mode).getName()));
					} else {
