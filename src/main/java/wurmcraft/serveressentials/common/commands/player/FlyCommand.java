    ...
    if (args.length > 0) {
      ...
      if (user != null) {
        ...
        if (!user.capabilities.allowFlying) {
          ...
          ChatHelper.sendMessageTo(player, Local.FLY_ENABLED_OTHER
              .replaceFirst("#", user.getDisplayName().getUnformattedText()));
          user.sendPlayerAbilities();
        } else {
          user.capabilities.allowFlying = false;
          ChatHelper.sendMessageTo(user, Local.FLY_DISABLED);
          ChatHelper.sendMessageTo(player, Local.FLY_DISABLED_OTHER
              .replaceFirst("#", user.getDisplayName().getUnformattedText()));
          user.sendPlayerAbilities();
        }
      }
