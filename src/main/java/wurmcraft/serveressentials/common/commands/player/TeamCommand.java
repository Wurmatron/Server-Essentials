...
public class TeamCommand extends SECommand {
  ...
  public void create(ICommandSender sender, String[] args) {
    EntityPlayer player = (EntityPlayer) sender.getCommandSenderEntity();
		if (args.length > 0) {
			Team team = new Team(Strings.join(Arrays.copyOfRange(args, 0, args.length), " "),
					player.getGameProfile().getId(), false);
			if (TeamManager.register(team)) {
				...
				ChatHelper.sendMessageTo(player, Local.TEAM_CREATED
						.replaceAll("#", Strings.join(Arrays.copyOfRange(args, 0, args.length), " ")));
			} else {
			...
			if (args.length == 1) {
				...
				if (user != null) {
					ChatHelper.sendMessageTo(player,
							Local.TEAM_INVITED.replaceAll("#", user.getDisplayName().getUnformattedText()));
      ...
      if (args.length == 1 && args[0] != null) {
				for (UUID key : UsernameCache.getMap().keySet()) {
					if (UsernameCache.getLastKnownUsername(key).equalsIgnoreCase(args[0])) {
						if (data == null) {
							DataHelper2.load(Keys.PLAYER_DATA, new PlayerData(key, null));
						}
						DataHelper2.forceSave(Keys.PLAYER_DATA, data);
						...
						ChatHelper.sendMessageTo(player,
								Local.TEAM_KICKED_OTHER.replaceAll("#", UsernameCache.getLastKnownUsername(key)));
					}
				}
      }
    } else if (team != null) {
  ...
  public void info(ICommandSender sender, String[] args) {
    ...
    if (team != null) {
      ...
      ChatHelper.sendMessageTo(player,
          TextFormatting.AQUA + "Owner: " + UsernameCache.getLastKnownUsername(team.getLeader()));
      ChatHelper.sendMessageTo(player, TextFormatting.AQUA + "Open: " + team.isPublic());
      ChatHelper.sendMessageTo(player,
          TextFormatting.AQUA + "Color: " + team.getTeamColor().getFriendlyName());
      if (team.getMembers().size() > 0) {
        ...
        ChatHelper
            .sendMessageTo(player, TextFormatting.AQUA + "Members: " + Strings.join(members, ", "));
      }
      ChatHelper.sendMessageTo(player, TextFormatting.RED + Local.SPACER);
    } else if (team == null) {
      ChatHelper.sendMessageTo(player, Local.TEAM_NONE);
  ...
  public void set(ICommandSender sender, String[] args) {
    ...
    if (team != null && team.getLeader().equals(player.getGameProfile().getId())) {
			if (args.length == 0) {
				...
				if (args.length > 1 && args[1].equalsIgnoreCase("true") || args[1]
						.equalsIgnoreCase("yes")) {
					team.setPublic(true);
					DataHelper2.forceSave(Keys.PLAYER_DATA, data);
				} else if (args.length > 1 && args[1].equalsIgnoreCase("false") || args[1]
						.equalsIgnoreCase("no")) {
					team.setPublic(false);
					DataHelper2.forceSave(Keys.PLAYER_DATA, data);
					ChatHelper.sendMessageTo(player,
							Local.TEAM_SET_VALUE.replaceAll("#", "public").replaceAll("$", args[1]));
				} else {
					ChatHelper.sendMessageTo(player,
							Local.TEAM_INVAID_VALUE.replaceAll("#", args[0]).replaceAll("$", args[1]));
				}
				...
				if (color != null) {
					team.setColor(color);
					DataHelper2.forceSave(Keys.PLAYER_DATA, data);
					ChatHelper.sendMessageTo(player, Local.TEAM_SET_VALUE.replaceAll("#", "color")
							.replaceAll("$", color.getFriendlyName()));
				} else {
					ChatHelper.sendMessageTo(player, getUsage(sender));
