  ...
  public void execute(MinecraftServer server, ICommandSender sender, String[] args)
    ...
    if (args.length > 1) {
      ...
      if (player != null && RankManager.getRankFromName(args[1]) != null) {
        ...
        String name =
            playerData.getNickname() != null ? "*" + TextFormatting.RESET + playerData.getNickname()
                .replaceAll("&", "\u00A7") : player.getDisplayNameString();
        ChatHelper.sendMessageTo(sender, Local.RANK_CHANGED.replaceAll("Your", name)
            .replaceAll("#", RankManager.getRankFromName(args[1]).getName()));
      }
      if (!found) {
        for (UUID id : UsernameCache.getMap().keySet()) {
          if (UsernameCache.getMap().get(id).equalsIgnoreCase(args[0])
              && RankManager.getRankFromName(args[1]) != null) {
            found = true;
            ...
            String name = playerData.getNickname() != null ? "*" + TextFormatting.RESET + playerData
                .getNickname().replaceAll("&", "\u00A7") : UsernameCache.getLastKnownUsername(id);
            ChatHelper.sendMessageTo(sender, Local.RANK_CHANGED.replaceAll("Your", name)
                .replaceAll("#", RankManager.getRankFromName(args[1]).getName()));
          }
        }
