...
public class PreGenCommand extends SECommand {
  ...
  public void execute(MinecraftServer server, ICommandSender sender, String[] args)
    ...
    if (sender.getCommandSenderEntity() instanceof EntityPlayer && args.length == 0) {
      ...
      if (worldGen == null) {
        worldGen = new WorldGenHandler(
            FMLCommonHandler.instance().getMinecraftServerInstance().worlds[player.dimension]
                .getChunkProvider(),
            FMLCommonHandler.instance().getMinecraftServerInstance().worlds[player.dimension], 20);
    ...
    if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
      ...
      if (args.length == 0) {
        ...
        try {
          ...
          if (checkBorderSize(
              FMLCommonHandler.instance().getMinecraftServerInstance().worlds[dim])) {
            ChatHelper.sendMessageTo(player, Local.PREGEN_WARN);
          }
          worldGen = new WorldGenHandler(
              FMLCommonHandler.instance().getMinecraftServerInstance().worlds[dim]
                  .getChunkProvider(),
              FMLCommonHandler.instance().getMinecraftServerInstance().worlds[dim], 20);
          ChatHelper.sendMessageTo(player, Local.PREGEN_STARTED);
        } catch (NumberFormatException e) {
          ChatHelper.sendMessageTo(sender.getCommandSenderEntity(),
              Local.INVALID_NUMBER.replaceAll("#", args[0]));
        }
        ...
        try {
          ...
          worldGen = new WorldGenHandler(
              FMLCommonHandler.instance().getMinecraftServerInstance().worlds[dim]
                  .getChunkProvider(),
              FMLCommonHandler.instance().getMinecraftServerInstance().worlds[dim], speed);
          ChatHelper.sendMessageTo(player, Local.PREGEN_STARTED);
        } catch (NumberFormatException e) {
          ChatHelper.sendMessageTo(sender.getCommandSenderEntity(),
              Local.INVALID_NUMBER.replaceAll("#", args[0]));
  ...
  public void pause(ICommandSender sender, String[] args) {
    ...
    ChatHelper.sendMessageTo(sender,
        Local.PREGEN.replaceAll("#", worldGen.running ? "Paused" : "Unpaused"));
  }
}
