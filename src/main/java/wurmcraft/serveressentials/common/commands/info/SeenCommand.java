			...
			if (args.length > 0) {
				...
				if (players.getPlayers().size() > 0) {
					for (EntityPlayerMP user : players.getPlayers()) {
						if (user.getGameProfile().getId()
								.equals(UsernameResolver.getPlayerUUID(resolvedUsername))) {
							ChatHelper.sendMessageTo(sender, (TextFormatting.GREEN + "Player: '" + UsernameCache
									.getLastKnownUsername(user.getGameProfile().getId()) + "' is currently online!"));
							displayed = true;
						}
					}
				}
				if (!displayed) {
					for (UUID username : UsernameCache.getMap().keySet()) {
						if (UsernameCache.getLastKnownUsername(username).equals(args[0])) {
							PlayerData data = DataHelper2.load(Keys.PLAYER_DATA, new PlayerData(username, null));
							if (data != null) {
								ChatHelper.sendMessageTo(sender, Local.LAST_SEEN.replaceAll("#",
										TextFormatting.DARK_AQUA + new Date(data.getLastseen()).toString()));
								displayed = true;
							}
							DataHelper2.delete(Keys.PLAYER_DATA, UsernameResolver.getPlayerData(username));
						}
					}
				}
				if (!displayed) {
