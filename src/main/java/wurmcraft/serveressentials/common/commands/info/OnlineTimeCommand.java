...
public class OnlineTimeCommand extends SECommand {
	...
	public void execute (MinecraftServer server,ICommandSender sender,String[] args) throws CommandException {
		...
		HashMap <UUID, PlayerData> dataMap = new HashMap <UUID, PlayerData> () {
			{
				if (args.length == 0) {
					UsernameCache.getMap ().forEach ((uuid,s) -> put (uuid,DataHelper2.load (Keys.PLAYER_DATA,new PlayerData (uuid,null))));
				} else {
					...
					for (String arg : args) {
						UsernameCache.getMap ().forEach ((uuid,s) -> {
							if (s.equalsIgnoreCase (arg))
								put (uuid,DataHelper2.load (Keys.PLAYER_DATA,new PlayerData (uuid,null)));
