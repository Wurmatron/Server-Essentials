...
public class HelpCommand extends SECommand {
	...
	private static final Map <String, ICommand> prunedAliases = pruneAliases (FMLCommonHandler.instance ().getMinecraftServerInstance ().getCommandManager ());
	...
	public void execute (MinecraftServer server,ICommandSender sender,String[] args) throws CommandException {
		...
		try {
			...
			if (start <= prunedAliases.size ()) {
				...
				b.append (Local.SPACER.substring (0,startPos - 1) + nPage).append (Local.SPACER.substring (0,chatWidth - b.length ()));
				if (start / 8 == 0)
					ChatHelper.sendMessageTo (sender,TextFormatting.RED + b.toString ());
				else
					ChatHelper.sendMessageTo (sender,TextFormatting.RED + b.toString (),clickEvent ((start / 8) - 1),null);
