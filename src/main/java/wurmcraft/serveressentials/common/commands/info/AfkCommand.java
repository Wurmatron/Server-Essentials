...
public class AfkCommand extends SECommand {
  ...
  public void execute(MinecraftServer server, ICommandSender sender, String[] args)
    ...
    if (DataHelper2.getTemp(Keys.AFK, player.getGameProfile().getId()) != null &&
        DataHelper2.getTemp(Keys.AFK, player.getGameProfile().getId())
            .get(player.getGameProfile().getId()) != null) {
      FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().sendMessage(
          new TextComponentString(Local.AFK_OFF.replaceAll("#", player.getDisplayNameString())));
      DataHelper2.addTemp(Keys.AFK, player.getGameProfile().getId(), false, true);
    } else {
      FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().sendMessage(
          new TextComponentString(Local.AFK_NOW.replaceAll("#", player.getDisplayNameString())));
