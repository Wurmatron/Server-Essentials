...
public class MailCommand extends SECommand {
  ...
  private static List<Integer> parseMailIndices(EntityPlayer player, String[] args,
      Mail[] playerMail) {
  ...
  public void list(ICommandSender sender, String[] args) {
    ...
    List<Mail> playerMail = ((PlayerData) DataHelper2.get(Keys.PLAYER_DATA,
        ((EntityPlayer) sender.getCommandSenderEntity()).getGameProfile().getId().toString()))
        .getMail();
		if (playerMail.size() > 0) {
			...
			for (int index = 0; index < playerMail.size(); index++) {
				ChatHelper.sendMessageTo(player,
						TextFormatting.GREEN + "[" + (index + 1) + "]: " + StringUtils
								.replaceEach(ConfigHandler.mailFormat, new String[]{"%username%", "%message%"},
										new String[]{TextFormatting.AQUA + UsernameResolver
												.getUsername(playerMail.get(index).getSender()),
												TextFormatting.GOLD + playerMail.get(index).getMessage()
    ...
    if (trailingArgs.length == 0) {
      ...
      if (trailingArgs.length == 1 && trailingArgs[0].equalsIgnoreCase("all")) {
        ...
        for (int index : parseMailIndices(player, trailingArgs, playerMail)) {
          ...
          ChatHelper
              .sendMessageTo(player, TextFormatting.DARK_AQUA + "Mail [" + index + "] deleted!");
        }
			...
			for (int index : parseMailIndices(player, trailingArgs, playerMail)) {
				ChatHelper.sendMessageTo(player,
						TextFormatting.GREEN + "[" + (index + 1) + "]: " + StringUtils
								.replaceEach(ConfigHandler.mailFormat, new String[]{"%username%", "%message%"},
										new String[]{TextFormatting.AQUA + UsernameResolver
												.getUsername(playerMail[index].getSender()),
												TextFormatting.GOLD + playerMail[index].getMessage()
														.replaceAll("&", "\u00A7")}));
			}
    }
  }
  ...
  public void send(ICommandSender sender, String[] args) {
    ...
    if (receiver == null) {
      ChatHelper.sendMessageTo(player, Local.PLAYER_NOT_FOUND
          .replaceAll(((uReceiver == null) ? "\"#\" " : "\"#\""),
              ((uReceiver == null) ? "" : uReceiver)));
      return;
    }
    ...
    playerData.addMail(new Mail(player.getGameProfile().getId(), uuidReceiver,
        Strings.join(CommandUtils.getArgsAfterCommand(1, trailingArgs), " ")));
