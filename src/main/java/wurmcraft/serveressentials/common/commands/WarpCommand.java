...
public class WarpCommand extends EssentialsCommand {
		...
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
				if (sender.getEntityWorld().isRemote) return; if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
						if (args.length == 0) execute(server, sender, new String[]{"list"});
						else {
								if (args[0].equalsIgnoreCase("list")) {
										List<String> warps = new ArrayList<>(); for (Warp warp : DataHelper.getWarps())
										...
										if (teleport_timer + (Settings.teleport_cooldown * 1000) <= System.currentTimeMillis()) {
												...
												text.getStyle().setHoverEvent(hoverEvent(warp)); player.addChatComponentMessage(text);
								...
								} else sender.addChatMessage(new TextComponentString(Local.WARP_NONE.replaceAll("#", args[0])));
						}
				}
		}
