...
public class TpaCommand extends EssentialsCommand {
		...
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
				if (sender instanceof EntityPlayer) {
						EntityPlayer player = (EntityPlayer) sender; if (args.length == 1) {
								PlayerList players = server.getServer().getPlayerList(); if (players.getPlayerList().size() > 0) {
										boolean found = false; for (EntityPlayerMP otherPlayer : players.getPlayerList())
												if (otherPlayer.getGameProfile().getId().equals(server.getServer().getPlayerProfileCache().getGameProfileForUsername(args[0]).getId())) {
														found = true; if (!activeRequests.values().contains(new EntityPlayer[]{player, otherPlayer})) {
										...
										if (!found) ((EntityPlayer) sender).addChatComponentMessage(new TextComponentString(Local.TPA_USER_NOTFOUND));
								}
						} else ((EntityPlayer) sender).addChatComponentMessage(new TextComponentString(Local.TPA_USERNAME_NONE));
				}
		}
}
