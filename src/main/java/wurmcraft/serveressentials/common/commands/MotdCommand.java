		...
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
				...
				if (globalSettings.getMotd() != null && globalSettings.getMotd().length > 0) {
						if (globalSettings.getMotd().length > LIST_SIZE) {
								if (args.length == 0) {
										String[] temp = Arrays.copyOfRange(globalSettings.getMotd(), 0, LIST_SIZE); for (String rule : temp)
												sender.addChatMessage(new TextComponentString(rule));
								} else {
										int pageNo = Integer.valueOf(args[0]); if (pageNo <= (globalSettings.getMotd().length / LIST_SIZE)) {
						...
						} else for (String rule : globalSettings.getMotd())
								sender.addChatMessage(new TextComponentString(rule));
				} else sender.addChatMessage(new TextComponentString(Local.NO_MOTD));
		}
}
