...
public class SetHomeCommand extends EssentialsCommand {
		...
		public List<String> getCommandAliases() {
				List<String> aliases = new ArrayList<>(); aliases.add("sethome"); aliases.add("sHome"); aliases.add("SetHome");
				aliases.add("shome"); return aliases;
		}

		@Override
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
				if (sender.getEntityWorld().isRemote) return; if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
						EntityPlayer player = (EntityPlayer) sender.getCommandSenderEntity(); if (args != null && args.length > 0) {
								...
								defaultHome.getStyle().setHoverEvent(hoverEvent(home)); sender.addChatMessage(defaultHome);
						} else {
								...
								nameHome.getStyle().setHoverEvent(hoverEvent(home)); sender.addChatMessage(nameHome);
						}
		...
		public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos pos) {
				List<String> list = new ArrayList<>(); if (args.length == 0) list.add(Settings.home_name); return list;
		}
