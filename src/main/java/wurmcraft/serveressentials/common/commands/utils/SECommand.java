  ...
  public boolean checkPermission(MinecraftServer server, ICommandSender sender) {
    if (ConfigHandler.securityModule && requiresTrusted()) {
      if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
        return SecurityUtils.isTrustedMember((EntityPlayer) sender.getCommandSenderEntity())
            && RankManager.hasPermission(((PlayerData) DataHelper2.get(Keys.PLAYER_DATA,
            ((EntityPlayer) sender.getCommandSenderEntity()).getGameProfile().getId().toString()))
            .getRank(), perm.toString());
      }
    } else if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
      return RankManager.hasPermission(((PlayerData) DataHelper2.get(Keys.PLAYER_DATA,
          ((EntityPlayer) sender.getCommandSenderEntity()).getGameProfile().getId().toString()))
          .getRank(), perm.toString());
    } else {
