...
public class KitCommand extends SECommand {
		...
		if (args.length == 1) {
			if (args[0].equalsIgnoreCase("list")) {
				...
				ChatHelper.sendMessageTo(player,
						TextFormatting.DARK_AQUA + "Kit: " + TextFormatting.AQUA + Strings.join(kitNames, " "));
			} else {
				if (DataHelper2.getData(Keys.KIT).size() > 0) {
					...
					if (!found) {
						ChatHelper.sendMessageTo(player,
								TextFormatting.RED + Local.KIT_NOTFOUND.replaceAll("#", args[0]));
					} else {
  ...
  private boolean addStack(EntityPlayer player, ItemStack stack) {
    if (stack.getItem() instanceof ItemArmor) {
      ...
      if (armor.armorType.equals(EntityEquipmentSlot.HEAD)
          && player.inventory.getStackInSlot(100) == null) {
        player.inventory.armorInventory.set(3, stack);
        return true;
      } else if (armor.armorType.equals(EntityEquipmentSlot.CHEST)
          && player.inventory.getStackInSlot(101) == null) {
        player.inventory.armorInventory.set(2, stack);
        return true;
      } else if (armor.armorType.equals(EntityEquipmentSlot.LEGS)
          && player.inventory.getStackInSlot(102) == null) {
        player.inventory.armorInventory.set(1, stack);
        return true;
      } else if (armor.armorType.equals(EntityEquipmentSlot.FEET)
          && player.inventory.getStackInSlot(103) == null) {
