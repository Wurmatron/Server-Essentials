		...
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
				...
				if (globalSettings.getRules() != null && globalSettings.getRules().length > 0) {
						if (globalSettings.getRules().length > LIST_SIZE) {
								if (args.length == 0) {
										String[] temp = Arrays.copyOfRange(globalSettings.getRules(), 0, LIST_SIZE); for (String rule : temp)
												sender.addChatMessage(new TextComponentString(rule));
								} else {
										int pageNo = Integer.valueOf(args[0]); if (pageNo <= (globalSettings.getRules().length / LIST_SIZE)) {
												String[] temp = Arrays.copyOfRange(globalSettings.getRules(), (pageNo * LIST_SIZE), (pageNo * LIST_SIZE) + LIST_SIZE);
												for (String rule : temp)
														if (rule != null && rule.length() > 0) sender.addChatMessage(new TextComponentString(rule));
										} else
						...
						} else for (String rule : globalSettings.getRules())
								sender.addChatMessage(new TextComponentString(rule));
				} else sender.addChatMessage(new TextComponentString(Local.NO_RULES));
		}
}
