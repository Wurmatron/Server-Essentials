...
public class ReplyCommand extends SECommand {
	...
	public void execute (MinecraftServer server,ICommandSender sender,String[] args) throws CommandException {
		...
		if (DataHelper2.getTemp (Keys.LAST_MESSAGE,player.getGameProfile ().getId (),player.getGameProfile ().getId ()) != null) {
			for (EntityPlayer entity : server.getPlayerList ().getPlayers ())
				if (entity.getGameProfile ().getId ().equals (DataHelper2.getTemp (Keys.LAST_MESSAGE,player.getGameProfile ().getId (),player.getGameProfile ().getId ())))
