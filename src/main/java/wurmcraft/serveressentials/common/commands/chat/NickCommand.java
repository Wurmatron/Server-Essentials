  ...
  public void execute(MinecraftServer server, ICommandSender sender, String[] args)
    ...
    if (args.length == 1) {
      ...
      if (player != null && data.getNickname() != null) {
        ...
        ChatHelper
            .sendMessageTo(sender, Local.NICK_NONE.replaceAll("#", player.getDisplayNameString()));
      } else {
      ...
      if (player != null) {
        ...
        ChatHelper.sendMessageTo(player,
            Local.NICKNAME_SET.replaceAll("#", args[1].replaceAll("&", "\u00A7")));
        if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
          ...
          if (!senderPlayer.getGameProfile().getId().equals(player.getGameProfile().getId())) {
            ChatHelper.sendMessageTo(senderPlayer,
                Local.NICKNAME_OTHER.replaceAll("#", args[1].replaceAll("&", "\u00A7")));
          }
        }
      } else {
        ChatHelper.sendMessageTo(sender, Local.PLAYER_NOT_FOUND.replaceAll("#", args[0]));
