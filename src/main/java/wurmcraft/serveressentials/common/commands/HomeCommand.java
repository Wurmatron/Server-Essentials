...
public class HomeCommand extends EssentialsCommand {
		...
		public List<String> getCommandAliases() {
				List<String> aliases = new ArrayList<>(); aliases.add("Home"); return aliases;
		}

		@Override
		public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
				if (sender.getEntityWorld().isRemote) return; if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
						EntityPlayerMP player = (EntityPlayerMP) sender.getCommandSenderEntity(); if (args.length == 0) {
								...
								if (home != null && (teleport_timer + (Settings.teleport_cooldown * 1000)) <= System.currentTimeMillis()) {
										...
										text.getStyle().setHoverEvent(hoverEvent(home)); sender.addChatMessage(text);
								...
								else sender.addChatMessage(new TextComponentString(Local.HOME_NONE));
						} else if (args.length == 1) {
								if (args[0].equalsIgnoreCase("list")) {
										PlayerData data = DataHelper.getPlayerData(player.getGameProfile().getId());
										if (data == null) DataHelper.reloadPlayerData(player.getGameProfile().getId());
										if (data.getHomes().length > 0) {
												ArrayList<String> homes = new ArrayList<>(); for (Home h : data.getHomes())
														if (h != null) homes.add(h.getName()); if (homes.size() > 0)
														sender.addChatMessage(new TextComponentString(TextFormatting.AQUA + Strings.join(homes.toArray(new String[0]), ", ")));
												else sender.addChatMessage(new TextComponentString(Local.HOME_NONEXISTENT));
										} else sender.addChatMessage(new TextComponentString(Local.HOME_NONEXISTENT));
								} else {
										...
										else sender.addChatMessage(new TextComponentTranslation(Local.HOME_INVALID.replace("#", args[0])));
								}
						}
