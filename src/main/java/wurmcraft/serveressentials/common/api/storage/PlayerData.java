...
public class PlayerData {
		...
		public Home getHome(String name) {
				if (homes != null) for (Home h : homes)
						if (h != null && h.getName().equalsIgnoreCase(name)) return h; return null;
		}

		public void setHomes(Home[] homes) {
				if (homes != null && homes.length > 0) this.homes = homes;
		}

		public String addHome(Home home) {
				if (home != null && homes != null && home.getName() != null) {
						boolean temp = false; for (int index = 0; index < homes.length; index++) {
								if (homes[index] != null && homes[index].getName().equalsIgnoreCase(home.getName())) {
										homes[index] = home; return Local.HOME_REPLACED.replaceAll("#", home.getName());
								} else if (homes[index] == null) {
										homes[index] = home; return Local.HOME_SET.replaceAll("#", home.getName());
								}
				...
				if (name != null && homes != null) {
						for (int index = 0; index < homes.length; index++) {
								if (homes[index] != null && homes[index].getName().equalsIgnoreCase(name)) {
										homes[index] = null; return Local.HOME_DELETED.replaceAll("#", name);
								}
						}
		...
		public IRank getRank() {
				IRank group = RankManager.getRankFromName(rank); if (group != null) return group;
				setRank(RankManager.getDefaultRank()); return RankManager.getDefaultRank();
		}

		public void setRank(String rank) {
				IRank group = RankManager.getRankFromName(rank); if (group != null) setRank(group);
				else setRank(RankManager.getDefaultRank());
		}

		public void setRank(IRank rank) {
				if (RankManager.getRanks().contains(rank)) this.rank = rank.getName();
				else this.rank = RankManager.getDefaultRank().getName();
		}

		public Team getTeam() {
				if (team != null) {
						Team playerTeam = TeamManager.getTeamFromName(team); if (playerTeam != null) return playerTeam;
						else playerTeam = null;
				} return null;
		}

		public void setTeam(ITeam t) {
				if (t != null) this.team = t.getName();
				else this.team = null;
		}
}
