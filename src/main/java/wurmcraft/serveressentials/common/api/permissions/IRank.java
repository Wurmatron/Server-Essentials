...
public interface IRank {
  ...
  /**
   * Name of this rank (Used for storage)
   */
  String getName();

  /**
   * Is default rank? more than 1 result in the one with the lower name when sorted alphabetical
   */
  boolean isDefault();
  ...
  /**
   * Used for power and permissions takes everything from its inheritanced groups
   */
  ...
  default boolean hasPermission(String perm) {
    for (String permission : getPermissions()) {
      if (permission.equalsIgnoreCase("*") || permission.endsWith("*") && permission
          .startsWith(perm.substring(0, perm.indexOf("."))) || permission.equalsIgnoreCase(perm)) {
        return true;
      }
    }
    return false;
  ...
  default boolean hasPermission(Perm perm) {
    for (String permission : getPermissions()) {
      if (permission.equalsIgnoreCase("*") || permission.endsWith("*") && permission
          .startsWith(perm.toString().substring(0, perm.toString().indexOf("."))) || permission
          .equalsIgnoreCase(perm.toString())) {
        return true;
      }
    }
    return false;
