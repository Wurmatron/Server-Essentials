...
public class ClaimEvent {
  ...
  public void onRightClickBlock(PlayerInteractEvent.RightClickBlock e) {
    ...
    if (claim != null && !ChunkManager
      ...
      e.setCanceled(true);
      ChatHelper.sendMessageTo(e.getEntityPlayer(),
          Local.CLAIM_INTERACT.replaceAll("#", getOwner(claim)));
    }
  }
        ...
        if (claim != null) {
          if (e.getExplosion().getExplosivePlacedBy() instanceof EntityPlayer) {
            ...
            if (!ChunkManager.canDestroy(claim, player.getGameProfile().getId())) {
              ChatHelper
                  .sendMessageTo(player, Local.CLAIM_EXPLOSION.replaceAll("#", getOwner(claim)));
              e.setCanceled(true);
            }
          } else {
            e.setCanceled(true);
