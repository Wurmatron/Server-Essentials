...
public class MarketEvent {
  ...
  public void onInteract(PlayerInteractEvent e) {
    if (e.getWorld().getTileEntity(e.getPos()) != null && e.getWorld()
        .getTileEntity(e.getPos()) instanceof TileEntitySign) {
      ...
      } else if (e.getWorld().getTileEntity(e.getPos()) != null && isInventory(e.getWorld(),
          e.getPos())) {
  ...
  private void setShopBuy(World world, BlockPos pos, ItemStack stack) {
    ...
    String[] text = getLines(world, pos);
		if (text[0].equalsIgnoreCase("[IBuy]")) {
			text[0] = TextFormatting.GOLD + "[" + TextFormatting.DARK_RED + TextFormatting.BOLD + "IBuy"
					+ TextFormatting.RESET + TextFormatting.GOLD + "]";
		} else if (text[0].equalsIgnoreCase("[ISell]")) {
			text[0] = TextFormatting.GOLD + "[" + TextFormatting.DARK_RED + TextFormatting.BOLD + "ISell"
					+ TextFormatting.RESET + TextFormatting.GOLD + "]";
			...
			text[0] =
					TextFormatting.GOLD + "[" + TextFormatting.AQUA + "Sell" + TextFormatting.GOLD + "]";
		}
		...
		for (int index = 0; index < player.inventory.mainInventory.size(); index++) {
			if (amountLeft <= 0) {
				...
				if (stack.getCount() <= player.inventory.mainInventory.get(index).getCount()) {
					player.inventory.mainInventory.get(index)
							.setCount(player.inventory.mainInventory.get(index).getCount() - stack.getCount());
  ...
  private boolean canSell(World world, BlockPos pos, ItemStack stack) {
    IInventory inv = (IInventory) world.getTileEntity(pos);
		for (int index = 0; index < inv.getSizeInventory(); index++) {
			return inv.getStackInSlot(index) == null || inv.getStackInSlot(index) != null && inv
					.getStackInSlot(index).isItemEqual(stack);
		}
    return false;
  }
			...
			if (amountLeft <= 0) {
				...
				if (stack.getCount() <= inv.getStackInSlot(index).getCount()) {
					inv.getStackInSlot(index)
							.setCount(inv.getStackInSlot(index).getCount() - stack.getCount());
  ...
  private void setChest(World world, BlockPos pos, BlockPos chestPos) {
    ...
    sign.getTileData()
        .setIntArray("pos", new int[]{chestPos.getX(), chestPos.getY(), chestPos.getZ()});
  }
  ...
  private void handleIBuySign(EntityPlayer player, World world, BlockPos pos) {
    ...
    if (isValid(world, pos)) {
			if (playerData.getMoney() >= getPrice(world, pos)) {
				if (addStack(player, getStack(world, pos))) {
					...
					ChatHelper.sendMessageTo(player, Local.PURCHASE.replaceAll("@", "" + getPrice(world, pos))
							.replaceAll("#",
									getStack(world, pos).getCount() + "x " + getStack(world, pos).getDisplayName()));
				} else {
    ...
    } else if (getPrice(world, pos) >= 0 && player.getHeldItemMainhand() != null
				&& getStack(world, pos) == null) {
    ...
    if (isValid(world, pos)) {
			if (playerData.getMoney() >= getPrice(world, pos)) {
				if (hasStack(player, getStack(world, pos))) {
					...
					ChatHelper.sendMessageTo(player, Local.ITEM_SOLD.replaceAll("#",
							getStack(world, pos).getCount() + "x " + getStack(world, pos).getDisplayName())
							.replaceAll("@", "" + getPrice(world, pos)));
					playerData.setMoney(playerData.getMoney() + getPrice(world, pos));
				} else {
					ChatHelper.sendMessageTo(player,
							Local.ITEM_NONE.replaceAll("#", getStack(world, pos).getDisplayName()));
				}
			} else {
				ChatHelper.sendMessageTo(player, Local.MONEY_NONE);
			}
  ...
  private void handleBuySign(EntityPlayer player, World world, BlockPos pos) {
			...
			if (canBuy(world, getChest(world, pos), getStack(world, pos))) {
				if (playerData.getMoney() >= getPrice(world, pos)) {
					if (addStack(player, getStack(world, pos))) {
						...
						playerData.setMoney(playerData.getMoney() - getPrice(world, pos));
						PlayerData ownerData = (PlayerData) DataHelper2
								.get(Keys.PLAYER_DATA, getOwner(world, pos).toString());
						ownerData.setMoney(ownerData.getMoney() + getPrice(world, pos));
						DataHelper2.forceSave(Keys.PLAYER_DATA, ownerData);
						DataHelper2.forceSave(Keys.PLAYER_DATA, playerData);
						ChatHelper.sendMessageTo(player,
								Local.PURCHASE.replaceAll("@", "" + getPrice(world, pos)).replaceAll("#",
										getStack(world, pos).getCount() + "x " + getStack(world, pos)
												.getDisplayName()));
					} else {
						ChatHelper.sendMessageTo(player, Local.PLAYER_INVENTORY_FULL);
					}
    ...
    } else if (getPrice(world, pos) >= 0 && player.getHeldItemMainhand() != null && player
				.getHeldItemMainhand().isItemEqual(linkStack)) {
  ...
  private void handleSellSign(EntityPlayer player, World world, BlockPos pos) {
    if (isValid(world, pos) && hasValidLink(world, pos) && canSell(world, getChest(world, pos),
        getStack(world, pos))) {
			if (hasStack(player, getStack(world, pos))) {
				consumeStack(player, getStack(world, pos));
  ...
  private void handleLinking(EntityPlayer player, World world, BlockPos pos) {
    if (shops.containsKey(player.getGameProfile().getId())) {
			if (getStackForSign(world, pos) != null) {
				...
				setOwner(world, shops.get(player.getGameProfile().getId()),
    ...
    } else if (player.getHeldItemMainhand() != null && player.getHeldItemMainhand()
				.isItemEqual(linkStack)) {
