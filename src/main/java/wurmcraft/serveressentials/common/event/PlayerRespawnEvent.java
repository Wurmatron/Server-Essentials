...
public class PlayerRespawnEvent {
  ...
  public void constructEvent(PlayerEvent.Clone e) {
    if (e.isWasDeath() && !e.isCanceled()) {
      if (ConfigHandler.respawnPoint.equalsIgnoreCase("spawn")) {
        teleportToSpawn(e.getEntityPlayer());
      } else if (ConfigHandler.respawnPoint.equalsIgnoreCase("home")) {
        PlayerData playerData = UsernameResolver
            .getPlayerData(e.getEntityPlayer().getGameProfile().getId());
        Home defaultHome = playerData.getHome(ConfigHandler.homeName);
        if (defaultHome != null) {
          e.getEntityPlayer()
              .setLocationAndAngles(defaultHome.getPos().getX(), defaultHome.getPos().getY(),
                  defaultHome.getPos().getZ(), defaultHome.getYaw(), defaultHome.getPitch());
          e.getEntityPlayer().dimension = defaultHome.getDimension();
        }
      }
    }
    if (suicideData.size() > 0
        && suicideData.get(e.getEntityPlayer().getGameProfile().getId()) != null) {
  ...
  private void teleportToSpawn(EntityPlayer player) {
    ...
    if (spawn != null) {
      player
          .setLocationAndAngles(spawn.location.getX(), spawn.location.getY(), spawn.location.getZ(),
