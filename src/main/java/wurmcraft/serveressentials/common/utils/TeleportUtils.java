...
public class TeleportUtils {
    ...
    if (player instanceof EntityPlayerMP && pos != null) {
      ...
      data.setLastLocation(
          new LocationWrapper((int) player.posX, (int) player.posY, (int) player.posZ,
              player.dimension));
      doTeleport((EntityPlayerMP) player, position.getX(), position.getY(), position.getZ(),
          player.dimension);
      if (timer) {
      ...
      if (player.dimension != dim) {
        FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList()
            .transferPlayerToDimension((EntityPlayerMP) player, dim,
                new CustomTeleporter(player.world.getMinecraftServer().getWorld(player.dimension),
  ...
  public static void doTeleport(EntityPlayerMP player, double x, double y, double z, int dimID) {
    if (player.dimension != dimID) {
      ...
      if (player.isEntityAlive()) {
        ...
        player.setLocationAndAngles(x + 0.5, y + 1, z + 0.5, player.rotationYaw,
      ...
      player.connection
          .setPlayerLocation(x + 0.5, y + 1, z + 0.5, player.rotationYaw, player.rotationPitch);
  ...
  public static boolean safeLocation(World world, BlockPos pos) {
    return world.getBlockState(pos.down()).getBlock() != Blocks.AIR && !(world
        .getBlockState(pos.down()).getBlock() instanceof BlockLiquid)
  ...
  public static boolean canTeleport(UUID uuid) {
    if (uuid != null) {
      ...
      return data.getTeleportTimer() + (ConfigHandler.teleportCooldown * 1000) <= System
          .currentTimeMillis();
    }
    return false;
  }
    ...
    if (TpaCommand.activeRequests.size() > 0) {
      for (EntityPlayer[] players : TpaCommand.activeRequests.values()) {
        if (!players[0].getGameProfile().getId().equals(player.getGameProfile().getId())) {
          TpaCommand.activeRequests
              .put(System.currentTimeMillis(), new EntityPlayer[]{player, other});
          return true;
        }
      }
    } else {
