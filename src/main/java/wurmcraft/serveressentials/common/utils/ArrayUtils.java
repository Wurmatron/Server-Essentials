...
/**
 ...
 ALL OF THIS CODE HAS BEEN COPIED FROM THE 'lexij' PROJECT, OWNED BY https://github.com/matthewacon
 */
public class ArrayUtils {
	...
	public static final class Range <T extends Number> {
		...
		public boolean isWithinRange (T arbitraryNumber) {
			return (arbitraryNumber.doubleValue () >= this.lower.doubleValue () && arbitraryNumber.doubleValue () <= this.upper.doubleValue ());
		}
		...
		public static <T extends Number> Range <T> getDifference (Range <T> first,Range <T> second) {
			return new Range <T> ((T) new Integer (Math.abs (first.lower.byteValue () - second.lower.byteValue ())),(T) new Integer (Math.abs (first.upper.byteValue () - second.upper.byteValue ())));
		}
	}
	...
	public static <T> T[] copyOver (T[] original,int off,int len,T[] receiver) throws IndexOutOfBoundsException {
		if (len >= 0) {
	...
	public static <T> T[] splice (T[] array,int start,int end) {
		...
		return (start > end) ? copyOver (array,start,-1 * end,workingArray) : copyOver (array,start,end + 1,workingArray);
	}
}
