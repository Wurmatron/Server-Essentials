...
public class DataHelper {
		...
		public static void registerPlayer(EntityPlayer player) {
				if (!loadedPlayers.containsKey(player.getGameProfile().getId())) {
						...
						if (data != null) loadedPlayers.put(player.getGameProfile().getId(), data);
						else {
		...
		private static void createPlayerFile(UUID name, PlayerData data) {
				if (!playerDataLocation.exists()) playerDataLocation.mkdirs();
				File playerFileLocation = new File(playerDataLocation + File.separator + name.toString() + ".json"); try {
		...
		public static PlayerData loadPlayerData(UUID name) {
				...
				if (playerFileLocation.exists()) {
						ArrayList<String> lines = new ArrayList<>(); try {
								BufferedReader reader = new BufferedReader(new FileReader(playerFileLocation)); String line;
								...
								temp = temp + lines.get(s); return gson.fromJson(temp, PlayerData.class);
				} return null;
		}

		public static void unloadPlayerData(UUID name) {
				if (loadedPlayers.containsKey(name)) loadedPlayers.remove(name);
		}

		public static void reloadPlayerData(UUID name) {
				unloadPlayerData(name); PlayerData data = loadPlayerData(name); loadedPlayers.put(name, data);
		}

		public static String addPlayerHome(UUID name, Home home) {
				PlayerData data = getPlayerData(name); if (data == null) data = loadPlayerData(name); if (data != null) {
						File   playerFileLocation = new File(playerDataLocation + File.separator + name.toString() + ".json");
						String msg                = data.addHome(home); try {
								Files.write(Paths.get(playerFileLocation.getAbsolutePath()), gson.toJson(data).getBytes());
		...
		public static String deleteHome(UUID name, String home) {
				PlayerData data = getPlayerData(name); if (data == null) data = loadPlayerData(name); if (data != null) {
						File   playerFileLocation = new File(playerDataLocation + File.separator + name.toString() + ".json");
						String msg                = data.delHome(home); try {
								Files.write(Paths.get(playerFileLocation.getAbsolutePath()), gson.toJson(data).getBytes());
								reloadPlayerData(name);
						} catch (IOException e) {
								e.printStackTrace();
		...
		public static void updateTeleportTimer(UUID name) {
				PlayerData data = getPlayerData(name); if (data == null) data = loadPlayerData(name); if (data != null) {
						File playerFileLocation = new File(playerDataLocation + File.separator + name.toString() + ".json");
						data.setTeleport_timer(System.currentTimeMillis()); try {
								Files.write(Paths.get(playerFileLocation.getAbsolutePath()), gson.toJson(data).getBytes());
								reloadPlayerData(name);
						} catch (IOException e) {
								e.printStackTrace();
		...
		public static void updateLastseen(UUID name) {
				PlayerData data = getPlayerData(name); if (data == null) data = loadPlayerData(name); if (data != null) {
						File playerFileLocation = new File(playerDataLocation + File.separator + name.toString() + ".json");
						data.setLastseen(System.currentTimeMillis()); try {
								Files.write(Paths.get(playerFileLocation.getAbsolutePath()), gson.toJson(data).getBytes());
								reloadPlayerData(name);
						} catch (IOException e) {
								e.printStackTrace();
		...
		public static void setTeam(UUID name, ITeam team) {
				...
				File       playerFileLocation = new File(playerDataLocation + File.separator + name.toString() + ".json");
				if (data == null) data = loadPlayerData(name); if (data != null) {
						data.setTeam(team); try {
								Files.write(Paths.get(playerFileLocation.getAbsolutePath()), gson.toJson(data).getBytes());
								reloadPlayerData(name);
						} catch (IOException e) {
								e.printStackTrace();
		...
		public static void addMail(Mail mail) {
				PlayerData data = getPlayerData(mail.getReciver()); boolean wasLoaded = true; if (data == null) {
						data = loadPlayerData(mail.getReciver()); wasLoaded = false;
				} if (data != null) {
						File playerFileLocation = new File(playerDataLocation + File.separator + mail.getReciver().toString() + ".json");
						data.addMail(mail); try {
								Files.write(Paths.get(playerFileLocation.getAbsolutePath()), gson.toJson(data).getBytes());
								if (wasLoaded) reloadPlayerData(mail.getReciver());
								else unloadPlayerData(mail.getReciver());
						} catch (IOException e) {
								e.printStackTrace();
						}
				}
		...
		public static ITextComponent displayLocation(Home home) {
				...
				text.getStyle().setColor(TextFormatting.GREEN); return text;
		}
		...
		public static String createWarp(Warp warp) {
				if (loadedWarps.size() <= 0) loadWarps(); if (!warpLocation.exists()) warpLocation.mkdirs();
				File warpFileLocation = new File(warpLocation + File.separator + warp.getName() + ".json"); try {
		...
		public static Warp[] getWarps() {
				if (loadedWarps.size() > 0) return loadedWarps.toArray(new Warp[0]); return new Warp[0];
		}
						...
						for (File file : warpLocation.listFiles())
								if (file.isFile() && file.getName().endsWith(".json")) {
										ArrayList<String> lines = new ArrayList<>(); try {
												...
												temp = temp + lines.get(s); Warp warp = gson.fromJson(temp, Warp.class);
										if (warp != null) loadedWarps.add(warp);
								}
				}
		}

		public static Warp getWarp(String name) {
				if (loadedWarps.size() <= 0) loadWarps(); if (loadedWarps.size() > 0 && name != null && name.length() > 0) {
		...
		public static void deleteWarp(Warp warp) {
				loadedWarps.remove(warp); for (File file : warpLocation.listFiles())
						if (file.isFile() && file.getName().equalsIgnoreCase(warp.getName() + ".json")) file.delete();
		}

		public static void createGlobal(Global global) {
				if (!saveLocation.exists()) saveLocation.mkdirs();
				File globalFile = new File(saveLocation + File.separator + "Global.json"); try {
						globalFile.createNewFile(); Files.write(Paths.get(globalFile.getAbsolutePath()), gson.toJson(global).getBytes());
				} catch (IOException e) {
						e.printStackTrace();
				}
		}
		...
		public static void loadGlobal() {
				File globalLocation = new File(saveLocation + File.separator + "Global.json"); if (globalLocation.exists()) {
						ArrayList<String> lines = new ArrayList<>(); try {
								BufferedReader reader = new BufferedReader(new FileReader(globalLocation)); String line;
								while ((line = reader.readLine()) != null) lines.add(line); reader.close();
						} catch (FileNotFoundException e) {
								e.printStackTrace();
						} catch (IOException e) {
								...
								temp = temp + lines.get(s); globalSettings = gson.fromJson(temp, Global.class);
				} else {
						createGlobal(new Global(null, new String[]{}, new String[]{})); loadGlobal();
				}
		}

		public static void overrideGlobal(Global global) {
				createGlobal(global); loadGlobal();
		}

		public static void loadRanks() {
				if (groupLocation.exists()) {
						if (groupLocation.listFiles().length <= 0) createDefaultRank(); for (File file : groupLocation.listFiles())
								if (file.isFile() && file.getName().endsWith(".json")) {
										ArrayList<String> lines = new ArrayList<>(); try {
												BufferedReader reader = new BufferedReader(new FileReader(file)); String line;
												while ((line = reader.readLine()) != null) lines.add(line); reader.close();
												...
												temp = temp + lines.get(s); Rank group = gson.fromJson(temp, Rank.class);
										if (group != null) RankManager.registerRank(group);
								}
				}
		}

		public static void createGroup(IRank group) {
				if (RankManager.getRanks().size() <= 0) loadRanks(); if (!groupLocation.exists()) groupLocation.mkdirs();
		...
		public static void reloadRanks() {
				RankManager.clearAllRanks(); loadRanks();
		}
		...
		public static void createDefaultRank() {
				if (!groupLocation.exists() || groupLocation.listFiles().length <= 0) {
						...
						createGroup(defaultGroup); createGroup(adminGroup); loadRanks();
				}
		}

		public static void createTeam(ITeam team, boolean forceNew) {
				if (TeamManager.getTeams().size() <= 0) loadAllTeams(); if (!teamLoction.exists()) teamLoction.mkdirs();
		...
		public static void loadAllTeams() {
				if (teamLoction.exists()) {
						for (File team : teamLoction.listFiles()) {
								ArrayList<String> lines = new ArrayList<>(); try {
										BufferedReader reader = new BufferedReader(new FileReader(team)); String line;
										while ((line = reader.readLine()) != null) lines.add(line); reader.close();
								} catch (FileNotFoundException e) {
										e.printStackTrace();
								} catch (IOException e) {
										...
										temp = temp + lines.get(s); TeamManager.register(gson.fromJson(temp, Team.class));
						}
				} else teamLoction.mkdirs();
		}
