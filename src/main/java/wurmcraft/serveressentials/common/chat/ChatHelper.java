...
public class ChatHelper {
  ...
  public static String format(String username, IRank rank, Channel channel, int dimension,
      ...
      if (rank.getSuffix() != null && !rank.getSuffix().equals("")) {
        format = StringUtils.replaceEach(ConfigHandler.chatFormat,
            new String[]{USERNAME_KEY, CHANNEL_KEY, MESSAGE_KEY, DIMENSION_KEY, RANK_PREFIX_KEY,
                ...
                team.getTeamColor() + team.getName() + TextFormatting.RESET});
      } else {
        format = StringUtils
  ...
  public static void sendMessageTo(ICommandSender sender, EntityPlayer reciver, String message) {
    ...
    if (sender != null
        && DataHelper2.getTemp(Keys.SPY, reciver.getGameProfile().getId()).size() > 0) {
      for (EntityPlayer uuid : sender.getServer().getPlayerList().getPlayers()) {
        for (EntityPlayer spy : sender.getServer().getPlayerList().getPlayers()) {
          if (spy.getGameProfile().getId().equals(uuid.getGameProfile().getId())) {
            sendMessageTo(spy,
                TextFormatting.RED + "[Spy] " + TextFormatting.DARK_AQUA + reciver.getName()
  ...
  public static void sendMessageTo(EntityPlayer sender, EntityPlayer reciver, String message) {
    PlayerData reciverData = (PlayerData) DataHelper2
        .get(Keys.PLAYER_DATA, reciver.getGameProfile().getId().toString());
    String reciverName =
        reciverData.getNickname() != null ? TextFormatting.GRAY + "*" + TextFormatting.RESET
            + reciverData.getNickname().replaceAll("&", "\u00A7") : reciver.getDisplayNameString();
    sendMessageTo(reciver, message);
      ...
      for (UUID uuid : DataHelper2.getTemp(Keys.SPY, sender.getGameProfile().getId()).keySet()) {
        for (EntityPlayer spy : sender.getServer().getPlayerList().getPlayers()) {
          if (spy.getGameProfile().getId().equals(uuid)) {
            sendMessageTo(spy,
                TextFormatting.RED + "[Spy] " + TextFormatting.DARK_AQUA + reciverName
                    + TextFormatting.GREEN + " <- " + message);
          }
        }
      }
  ...
  public static void sendMessage(String displayName, IRank rank, Channel channel, int dimension,
      Team team, String message) {
    if (ConfigHandler.logChat) {
      LogHelper.addChat(channel,
          "[" + new Date(System.currentTimeMillis()).toString() + "] " + TextFormatting
              .getTextWithoutFormattingCodes(displayName) + " " + message);
  ...
  public static void sendChannelMessage(Channel channel, String displayName, IRank rank,
      int dimension, Team team, String message) {
    ...
    if (!channel.getName().equalsIgnoreCase("Team")) {
      ...
      if (team != null) {
        ...
        List<UUID> recivers = ChannelManager.getPlayersInChannel(channel);
        for (EntityPlayerMP player : players.getPlayers()) {
          if (recivers.contains(player.getGameProfile().getId()) && ((PlayerData) DataHelper2
              .get(Keys.PLAYER_DATA, player.getGameProfile().getId().toString())).getTeam()
              == null) {
            player.sendMessage(new TextComponentString(
                format(displayName, rank, channel, dimension, team, message)));
          }
  ...
  public static ITextComponent displayLocation(Warp warp) {
    TextComponentString text = new TextComponentString(
        "X = " + warp.getPos().getX() + " Y = " + warp.getPos().getY() + " Z = " + warp.getPos()
            .getZ());
    text.getStyle().setColor(TextFormatting.GREEN);
    return text;
  }
  ...
  public static ITextComponent displayLocation(SpawnPoint spawn) {
    TextComponentString text = new TextComponentString(
        "X = " + spawn.location.getX() + " Y = " + spawn.location.getY() + " Z = " + spawn.location
            .getZ());
    text.getStyle().setColor(TextFormatting.GREEN);
    return text;
  }
