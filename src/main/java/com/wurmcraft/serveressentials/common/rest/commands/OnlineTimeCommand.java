      ...
      if (args.length == 1) {
        ...
        if (player != null) {
          ...
          LocalUser local = (LocalUser) UserManager
              .getPlayerData(player.getGameProfile().getId())[1];
          sender.sendMessage(new TextComponentString(getCurrentLanguage(sender).CHAT_SPACER));
          String formattedGlobal = DurationFormatUtils
              .formatDuration(global.getOnlineTime() * 60000, "d%:H$:m#:s@")
              .replace('%', 'D').replace('$', 'H').replace('#', 'M').replace('@', 'S')
              .replaceAll(":", ", ");
          String formattedLocal = DurationFormatUtils
              .formatDuration(local.getOnlineTime() * 60000, "d%:H$:m#:s@")
              .replace('%', 'D').replace('$', 'H').replace('#', 'M').replace('@', 'S')
              .replaceAll(":", ", ");
          sender.sendMessage(new TextComponentString(
              getCurrentLanguage(sender).GLOBAL + ": " + TextFormatting.AQUA +
                  formattedGlobal));
          sender.sendMessage(new TextComponentString(
              getCurrentLanguage(sender).LOCAL + ": " + TextFormatting.AQUA +
                  formattedLocal));
          sender.sendMessage(new TextComponentString(getCurrentLanguage(sender).CHAT_SPACER));
        } else {
          sender.sendMessage(new TextComponentString(
      ...
      if (args.length == 1) {
        ...
        if (player != null) {
          PlayerData data = (PlayerData) UserManager
              .getPlayerData(player.getGameProfile().getId())[0];
          String formattedLocal = DurationFormatUtils
              .formatDuration(data.getOnlineTime() * 60000, "d%:H$:m#:s@")
              .replace('%', 'D').replace('$', 'H').replace('#', 'M').replace('@', 'S')
              .replaceAll(":", ", ");
          sender.sendMessage(new TextComponentString(
