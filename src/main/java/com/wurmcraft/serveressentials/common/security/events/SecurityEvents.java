  ...
  public void onJoinEvent(PlayerEvent.PlayerLoggedInEvent e) {
    if (ConfigHandler.autoOP
      ...
      ChatHelper.sendMessage(
          e.player, LanguageModule.getLangfromUUID(e.player.getGameProfile().getId()).TRUSTED_USER);
    }
      ...
      for (EntityPlayerMP player :
        ...
        if (cache.containsKey(player.connection.netManager.getRemoteAddress())) {
          ChatHelper.notifyStaff(
              LanguageModule.getLangFromKey(ConfigHandler.defaultLanguage).POSSIBLY_ALT);
        }
      }
    }
  }
