...
public class WorldEvent {
  ...
  public void onJoinEvent(PlayerEvent.PlayerLoggedInEvent e) {
    boolean newPlayer = DataHelper.createIfNonExist(Keys.PLAYER_DATA,
        new PlayerData(e.player.getGameProfile().getId(), UserManager.getDefaultRank()));
    PlayerData playerData = (PlayerData) DataHelper
  ...
  public void onLeaveEvent(PlayerEvent.PlayerLoggedOutEvent e) {
    PlayerData data = (PlayerData) DataHelper
        .get(Keys.PLAYER_DATA, e.player.getGameProfile().getId().toString());
    if (data != null) {
      ...
      DataHelper.forceSave(Keys.PLAYER_DATA,
          DataHelper.get(Keys.PLAYER_DATA, e.player.getGameProfile().getName()));
    }
  }
