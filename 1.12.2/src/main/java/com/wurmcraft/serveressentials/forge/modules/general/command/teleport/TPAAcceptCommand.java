...
public class TPAAcceptCommand {
  ...
  public void accept(ICommandSender sender) {
    if (SERegistry.isModuleLoaded("Rank") && RankUtils
        .hasPermission(RankUtils.getRank(sender), "general.tpaccept") || !SERegistry
        .isModuleLoaded("Rank")) {
      if (sender != null && sender.getCommandSenderEntity() != null && sender
          .getCommandSenderEntity() instanceof EntityPlayer) {
        EntityPlayer player = (EntityPlayer) sender.getCommandSenderEntity();
        try {
          TPARequest request = (TPARequest) SERegistry
              .getTempData(DataKey.TPA, player.getGameProfile().getId().toString());
          player
              .sendMessage(TextComponentUtils.addPlayerComponent(new TextComponentString(
                  COMMAND_COLOR + PlayerUtils
                      .getUserLanguage(player).GENERAL_TPACCEPT_ACCEPT
                      .replaceAll("%PLAYER%", COMMAND_INFO_COLOR + request.sendingPlayer
                          .getDisplayNameString())), request.sendingPlayer));
          request.sendingPlayer.sendMessage(TextComponentUtils.addPlayerComponent(
              new TextComponentString(COMMAND_COLOR + PlayerUtils
                  .getUserLanguage(request.sendingPlayer).GENERAL_TPACCEPT_SEND
                  .replaceAll("%PLAYER%",
                      COMMAND_INFO_COLOR + player.getDisplayNameString())), player));
          TeleportUtils.teleportTo(request.sendingPlayer,
              new LocationWrapper(request.requestedPlayer.posX,
                  request.requestedPlayer.posY, request.requestedPlayer.posZ,
                  request.requestedPlayer.dimension));
          SERegistry.delTempData(DataKey.TPA, request.getID());
        } catch (NoSuchElementException e) {
          sender.sendMessage(new TextComponentString(
              PlayerUtils.getUserLanguage(sender).GENERAL_TPAACCEPT_NOT_FOUND));
        }
      }
    } else {
      sender.sendMessage(new TextComponentString(
