...
public class TestSERegistry {
  ...
  private List<String> modulesThatShouldBeLoaded = Arrays
      .asList("TestA", "TestB", "TestC", "TestD");
  ...
  public void TestLoadedModulesArray() {
    ...
    assertTrue(modulesThatShouldBeLoaded.size() == loadedModules.size()
        && modulesThatShouldBeLoaded.containsAll(loadedModules) && loadedModules
        .containsAll(modulesThatShouldBeLoaded));
  }
  ...
  public void TestGetModuleB() {
    Class<?> module = SERegistry.getModule("TestB");
    Field[] field = module.getDeclaredFields();
    try {
      String moduleField = "";
      if (field.length > 0) {
      ...
      System.out
          .println("Field Data in the TestSERegistry#TestGetModule test!");
    }
  }
